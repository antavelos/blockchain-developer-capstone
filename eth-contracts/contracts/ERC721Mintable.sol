...
contract Ownable {
    ...
    modifier onlyOwner() {
        require(isOwner(msg.sender), "not contract owner");
        _;
    }
...
//    @dev Approves another address to transfer the given token ID
    function approve(address to, uint256 tokenId) public {
        ...
        require(to != ownerOf(tokenId), "owner already owns the token");

        address owner = getOwner();
        require(isOwner(msg.sender) || isApprovedForAll(owner, msg.sender), "not owner or not approved by owner");
    ...
    function _transferFrom(address from, address to, uint256 tokenId) internal {
        ...
        require(from == ownerOf(tokenId), "'from' address does not own the token");
...
contract CapstoneRealEstate is ERC721Metadata {

    constructor()
    public
    ERC721Metadata("Capstone Real Estate", "CRS", "https://s3-us-west-2.amazonaws.com/udacity-blockchain/capstone/")
    { }
